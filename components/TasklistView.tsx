      ...
      <AnimatePresence>
        {selectedTaskIds.length > 0 && (
          <motion.div 
            ...
            exit={{ y: 50, opacity: 0 }}
            className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-slate-800 text-white px-6 py-3 rounded-full flex items-center gap-4 shadow-2xl z-50"
          >
            <span className="font-semibold text-sm whitespace-nowrap">{selectedTaskIds.length} đã chọn</span>
            <div className="h-4 w-px bg-slate-600"></div>
            <button onClick={handleBulkComplete} className="text-blue-400 hover:text-blue-300 text-sm font-semibold transition-colors">Đánh dấu Hoàn thành</button>
            <div className="h-4 w-px bg-slate-600"></div>
            <button onClick={handleBulkDelete} className="text-red-400 hover:text-red-300 text-sm font-semibold transition-colors">Xóa</button>
            <button onClick={() => setSelectedTaskIds([])} className="ml-2 hover:bg-slate-700 p-1.5 rounded-full transition-colors"><XIcon className="w-4 h-4 text-slate-400 hover:text-white" /></button>
          </motion.div>
        )}
      </AnimatePresence>
